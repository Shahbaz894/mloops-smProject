...
def apply_bow(train_data: pd.DataFrame, test_data: pd.DataFrame, max_feature: int) -> tuple:
    try:
        ...
        X_test_bow = vectorize.transform(X_test)  # Changed to transform
        ...
        pickle.dump(vectorize, open('models/vectorizer.pkl', 'wb'))
...
def main():
    try:
        ...
        test_data = load_data('./data/interim/test_processed_data.csv')  # Updated filename
